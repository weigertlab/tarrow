...
class Resnet(nn.Module):
    ...
    def __init__(
        ...
        try:
            ...
            raise ValueError(
                "kernel_sizes expected to be a 3-level nested"
        ...
        try:
            ...
            raise ValueError(
                "downsample factors expected to be a 2-level"
                " nested tuple(conv block, spatial dims)"
            )

        if not (len(kernel_sizes) == 1 or len(kernel_sizes) == len(downsample_factors)):
            raise ValueError(
                "kernel_sizes and downsamle_factors dimensionalities do not correspond."
            )
        ...
        for k in kernel_sizes:
            if not np.all(
                np.logical_or((np.array(k) - 1) / 2 == padding, np.array(k) == 1)
            ):
    ...
    def forward(self, x):
        if x.ndim != self.dims + 2:
            raise ValueError(f"{x.ndim}D input given, {self.dims + 2}D input expected.")
...
class Resnet2d(Resnet):
    ...
    def __init__(self, *args, **kwargs):
        ...
        kwargs["kernel_sizes"] = (tuple((k, k) for k in kwargs["kernel_sizes"]),)
