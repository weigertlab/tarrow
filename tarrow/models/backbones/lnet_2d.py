...
class MaxPool2dDividing(nn.MaxPool2d):
    ...
    def forward(self, input):
        if torch.any(
            torch.as_tensor(input.shape[-2:], device=input.device)
            % torch.as_tensor(self.kernel_size, device=input.device)
            != 0
        ):
            raise ValueError(
                (
...
class Lnet2d(nn.Module):
    ...
    def __init__(
        ...
        assert (
            len(downsample_factors) == len(upsample_factors) + 1
        ), "Arbitrary combinations of up/downsample not implemented."
        for uf in upsample_factors:
            if not isinstance(uf, tuple):
                raise ValueError("Upsample factors have to be a list of tuples.")
            if not np.all(np.array(uf) == d[0]):
                raise NotImplementedError("Anisotropic upsampling not implemented.")
        # TODO fix
        # self.upsample_factors = upsample_factors + (
        # ((1, 1),) * (len(downsample_factors) - len(upsample_factors) - 1)
        # )
        self.upsample_factors = upsample_factors
        ...
        self.l_conv = nn.ModuleList(
            [
                ConvBlock(
                    ...
                    strides=(downsample_factors[level],)
                    + ((1, 1),) * (len(kernel_sizes) - 1),
        ...
        if constant_upsample:
            self.r_up = nn.ModuleList(
                [
                    nn.Upsample(
                        scale_factor=self.upsample_factors[level], mode="nearest"
                    )
    ...
    def rec_forward(self, level, f_in):
        ...
        if level == 0:
            ...
            # nested levels
            r_in = self.rec_forward(level - 1, l_conv)
    ...
    def crop(self, x, shape):
        ...
        shape = tuple(
            torch.minimum(torch.as_tensor(shape), torch.as_tensor(x.shape[-2:]))
        )
    ...
    def is_valid_input_size(self, size):
        ...
        us_factors = [np.array(x, dtype=np.int_) for x in self.upsample_factors]
